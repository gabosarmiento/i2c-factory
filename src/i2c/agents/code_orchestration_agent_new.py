...
class OrchestrationContextBuilder:
    ...
    def _make_cache_key(self, query: str, chunk_count: int) -> str:
        return f"{query.strip().lower()}_{chunk_count}"

    def retrieve_context(self, query: str, chunk_count: int = None) -> str:
        """Retrieve context from knowledge base with in-memory caching."""
        if not self.knowledge_base or not query:
            return ""

        chunk_count = chunk_count or self.default_chunk_count
        cache_key = self._make_cache_key(query, chunk_count)

        if cache_key in self.knowledge_cache:
            canvas.info(f"[RAG] Using cached context for query: {query[:50]}...")
            return self.knowledge_cache[cache_key]

        try:
            if len(query.split()) < 8:
                chunk_count = min(chunk_count, 2)

            canvas.info(f"[RAG] Retrieving context for: {query[:100]}...")
            chunks = self.knowledge_base.retrieve_knowledge(query=query, limit=chunk_count)
            ...
            self.knowledge_cache[cache_key] = context_text
            return context_text
    ...
    def retrieve_orchestration_context(self, objective: Dict[str, Any]) -> str:
        """Legacy method to support old signature."""
...
class CodeOrchestrationAgent(Agent):
    def __init__(
        ...
        *, 
        ...
        knowledge_base=None,  # RAG Integration: Add knowledge_base parameter
        **kwargs,
    ) -> None:
        # ------------------------------------------------------------------
        # 1. AGNO initialization (name, model, role, instructions)
        # ------------------------------------------------------------------
        ...
        # RAG Integration: Initialize context builder with knowledge base
        self.context_builder = OrchestrationContextBuilder(
            knowledge_base=knowledge_base,
            default_chunk_count=5,
            max_tokens=8000  # Higher token limit for orchestration
        )
    ...
    async def execute(self, objective: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # RAG Integration: Retrieve orchestration context
            orchestration_context = ""
            if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                orchestration_context = self.context_builder.retrieve_orchestration_context(objective)
                if orchestration_context:
                    canvas.success(f"[RAG:Orchestration] Retrieved context for task: {task[:50]}...")
                    
                    # Add to session state for other components
                    if self.session_state is not None:
                        self.session_state["orchestration_context"] = orchestration_context
    ...
    async def _setup_teams(self, project_path: Path):
        ...
        self.modification_team = build_code_modification_team(session_state=shared_session)
    ...
    async def _analyze_project_context(self, project_path: Path, task: str) -> Dict[str, Any]:
        ...
        try:
            # RAG Integration: Add team-specific context for analysis
            analysis_context = ""
            if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                analysis_context = self.context_builder.retrieve_team_specific_context(
                    "Analysis", 
                    f"project architecture analysis for {task}"
                )
            ...
            # 2. Architectural Intelligence Analysis (with robust error handling)
            try:
                from i2c.agents.architecture.architecture_understanding_agent import get_architecture_agent
                
                # RAG Integration: Pass retrieved context to architecture agent session state
                agent_session = {"analysis_context": analysis_context} if analysis_context else None
                
                architecture_agent = get_architecture_agent(agent_session)
                if architecture_agent:
                    ...
                    if hasattr(structural_context, 'architecture_pattern'):
                        ...
                        if hasattr(structural_context, 'modules'):
                            for name, module in structural_context.modules.items():
                                analysis_result["architectural_context"]["modules"][name] = {
                        ...
                        module_count = len(analysis_result["architectural_context"].get("modules", {}))
                        pattern = analysis_result["architectural_context"].get("architecture_pattern", "unknown")
            ...
            # 3. RAG Integration: Add retrieved knowledge to analysis result
            if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                analysis_result["context"]["knowledge_context"] = analysis_context
    ...
    async def _create_modification_plan(
            ...
            try:
                ...
                # RAG Integration: Retrieve planning-specific knowledge
                planning_context = ""
                if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                    # Build sub-queries for planning context
                    sub_queries = [
                        f"{language} code modification best practices",
                        f"planning software changes for {task[:50]}"
                    ]
                    
                    # For complex tasks, add more specialized sub-queries
                    if "refactor" in task.lower() or "restructure" in task.lower():
                        sub_queries.append("code refactoring patterns and techniques")
                    if "performance" in task.lower() or "optimize" in task.lower():
                        sub_queries.append("software performance optimization techniques")
                    if "testing" in task.lower() or "test" in task.lower():
                        sub_queries.append("test-driven development best practices")
                    
                    # Retrieve composite context
                    planning_context = self.context_builder.retrieve_composite_context(
                        main_query=f"planning code modifications for {task}",
                        sub_queries=sub_queries
                    )
                    
                    if planning_context:
                        canvas.info(f"[RAG:Planning] Retrieved context for planning: ~{len(planning_context)//4} tokens")
                ...
                # RAG Integration: Add retrieved knowledge to the planning context
                if planning_context:
                    enhanced_context = f"""
    # RELEVANT KNOWLEDGE FOR PLANNING
    The following knowledge is relevant to planning this modification:
    ...
    async def _run_quality_checks(self, modification_result: Dict[str, Any], quality_gates: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                ...
                sub_queries += [f"{gate} best practices" for gate in quality_gates[:3]]
            ...
            # Prepare content data cleanly before JSON dump
            content_data = {
    ...
    async def _run_operational_checks(self, modification_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # RAG Integration: Retrieve SRE-specific knowledge
            sre_context = ""
            if hasattr(self, 'context_builder') and self.context_builder.knowledge_base:
                # Build targeted query for SRE
                sre_query = "site reliability engineering best practices"
                
                # Add file-type specific SRE queries
                file_extensions = set()
                for file_path in modified_files.keys():
                ...
                sub_queries = [f"{ext} deployment safety checks" for ext in file_extensions]
                
                # Retrieve knowledge
                sre_context = self.context_builder.retrieve_team_specific_context(
                    team_name="SRE",
                    team_goal=sre_query
                )
                
                if sre_context:
                    canvas.info(f"[RAG:SRE] Retrieved context for operational checks: ~{len(sre_context)//4} tokens")
                    
                    # Add to session state for SRE team
                    if self.session_state is not None:
                        self.session_state["sre_context"] = sre_context
            ...
            # RAG Integration: Add SRE context to message if available
            if sre_context:
                message_payload["content"]["sre_context"] = sre_context
    ...
    def _clean_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Clean and prepare results for JSON serialization"""
        ...
        for key, value in results.items():
            # Skip any function references or complex objects
            if key == "debug_raw_output":
                continue
                
            # Handle nested dictionaries
            if isinstance(value, dict):
                clean_results[key] = self._clean_results(value)
            # Handle lists
            elif isinstance(value, list):
                clean_results[key] = [item if not callable(item) else str(item) for item in value]
    ...
    def _detect_primary_language(self, plan: Dict[str, Any]) -> str:
        """Detect primary language from plan"""
        # Try to get language from session state first
        language = self.session_state.get("language")
        if language:
            return language
            
        # Look for language in the architectural guidance
        arch_guidance = plan.get("architectural_guidance", {})
        modules = arch_guidance.get("module_boundaries", [])
        
        # Check for common module names
        if "frontend" in modules:
            return "javascript"
        elif "backend" in modules:
            return "python"
        
        # Look at target files
    ...
    def _get_content_samples(self, project_path: Path, files: List[str]) -> Dict[str, str]:
        """Get content samples for a list of files"""
        content_samples = {}
        ...
        for file_path in files:
            try:
                ...
                if full_path.is_file():
                    ...
                    # Limit content size to avoid overloading context
                    content_samples[file_path] = content[:1000] + "..." if len(content) > 1000 else content
            except Exception as e:
