...
def test_pipeline_when_validator_fails(monkeypatch, sample_file):

    dummy_agent = Agent(name="Dummy", role="test", goal="validator check")

    def fake_analyze(self, request):
        return AnalysisResult(details="safe")

    def fake_modify(self, request, analysis):
        # Normal-looking payload
        new_code = sample_file.read_text() + "\nprint('done')\n"
        payload = ModPayload(
            file_path=sample_file.name,
            original=sample_file.read_text(),
            modified=new_code
        )
        return ModificationPlan(diff_hints=json.dumps(payload.__dict__))

    def fake_validate(self, request, plan, analysis):
        # Simulate validator blocking the change
        return ValidationReport(ok=False, messages=["Security risk detected in code"])
    ...
    step = {"dummy": "check security rule"}
    ...
    assert "Security risk" in validation.messages[0]
